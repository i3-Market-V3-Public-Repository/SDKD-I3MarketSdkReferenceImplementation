...
public class SdkRiHub {
	...
	@ApiOperation(value = "Get SDK-RI configuration info ", tags = "common-services: management")
	...
	public String getConfigurationInfo(
		...
		configJson.put("data_access", SdkRiConstants.DATA_ACCESS_ENDPOINT);
	...
	// })
	// @Produces({ "application/json", "application/xml" })
	// public com.i3m.api.ApiResponse<JWKSet>
	// runOidcExample(@QueryParam("back_token") String jwt
	//
	// ) throws Exception {
	//
	...
	public com.i3m.api.ApiResponse<Void> registerDataProvider(@RequestBody DataProvider dataProvider)
			throws ApiException {
	...
	@Produces({ "application/json", "application/xml" })
	@Consumes(MediaType.APPLICATION_JSON)
	public com.i3m.api.ApiResponse<Void> registerDataOffering(@RequestBody DataOffering dataOffering)
			throws ApiException {
	...
	public String retrieveDataOfferingById(@QueryParam("offering_id") String id,
			@QueryParam("page") @DefaultValue("0") Integer page, @QueryParam("size") @DefaultValue("10") Integer size,
		...
		try {
			strJson = mapper.writerWithDefaultPrettyPrinter()
					.writeValueAsString(new RetrieveOfferingById().getDataOfferingById(id, page, size, sort));
	...
	@ApiResponses(value = {
			@ApiResponse(code = 400, message = "failed to retrieve offerings registered by this user") })
	@Produces({ "application/json", "application/xml" })
	public String retrieveAllDataOfferingsByProviderId(@QueryParam("provider_id") String id,
		...
		try {
			strJson = mapper.writerWithDefaultPrettyPrinter().writeValueAsString(
					new RetrieveOfferingByProviderId().getOfferingByProviderId(id, page, size, sort));
		} catch (ProcessingException | JsonProcessingException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		return strJson;
		// return new RetrieveOfferingByProviderId().getOfferingByProviderId(id,
		// page,size,sort);
	}
	...
	public String retrieveDataOfferingByCategory(@QueryParam("category") String category,
		...
		try {
			strJson = mapper.writerWithDefaultPrettyPrinter().writeValueAsString(
					new RetrieveOfferingByCategory().getOfferingByCategory(category, page, size, sort));
		} catch (JsonProcessingException | ProcessingException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		return strJson;
		// return mapper.readValue(new
		// RetrieveOfferingByCategory().getOfferingByCategory(category, page, size,
		// sort).toString(), List.class);
		// return new RetrieveOfferingByCategory().getOfferingByCategory(category, page,
		// size, sort);
	}
